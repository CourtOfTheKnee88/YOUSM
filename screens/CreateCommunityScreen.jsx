...
import { StyleSheet, Text, View, TextInput, Pressable, SafeAreaView, Alert } from "react-native";
...
export default function CreateCommunityScreen({ navigation }) {
  ...
  const [type, setType] = useState("Club"); // Default from Esther's data
  const [category, setCategory] = useState("Social"); // Default from Esther's data
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <Pressable
          style={[styles.button, (!name.trim() || !description.trim() || !type.trim() || !category.trim()) && { opacity: 0.5 }]}
          onPress={handleSubmit}
          disabled={!name.trim() || !description.trim() || !type.trim() || !category.trim()}
        >
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 16, fontWeight: "bold", color: COLORS.primary, marginBottom: 8, marginTop: 15 },
  input: {
  ...
  hint: { fontSize: 12, color: COLORS.textLight, marginTop: 8, marginBottom: 30 },
